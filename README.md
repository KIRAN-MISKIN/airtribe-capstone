...
curl -X POST http://localhost:3000/jobs \
  ...
  -d '{
    ...
    "scheduleAt": "2026-05-09T10:00:00.000Z",
    "priority": 3,
    "tags": ["demo"],
    "maxAttempts": 3
  }'
...
*Note: The `curl` commands below use standard bash formatting, which can be easily imported into **Postman**.*
...
**cURL:**
```bash
curl -X DELETE http://localhost:3000/jobs/<job-uuid> \
  -H "Authorization: Bearer <your-token>"
```
...
**cURL:**
```bash
curl -X PATCH http://localhost:3000/jobs/<job-uuid> \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <your-token>" \
  -d '{
    "scheduleAt": "2026-05-20T14:00:00Z",
